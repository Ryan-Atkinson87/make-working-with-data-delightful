...
# returns the mean of the values broken down into 6 hour periods

# .min() or .max() return the minimum or maximum temperatures for each interval
# .sum also gives the sums of data values

# temp.rolling(window=3).mean()
# returns a rolling average, in this case, of 3 adjacent rows

# temp.plot()
# uses 'pandas.DataFrame.plot() to create a plot

# plt.show()
# shows the created plot

# temp.plot().get_figure().savefig('temperatures.png')
# this statement creates the plot and saves it as a file
# called 'temperatures.png'

# df.loc[:, ['py-score', 'total']].plot.hist(bins=5, alpha=0.4)
# plot.hist creates a histogram of the 'py-score' and 'total'
# these plots give the basic look, you can adjust the details using matplots
# parameter, Anatomy of Matplotlib shows details of this
